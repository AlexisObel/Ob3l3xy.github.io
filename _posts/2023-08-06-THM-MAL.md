...
aims to affect as mane devices as possible and affect it in any kind of way.
...
it’s encrypting files, preventing certain services from being available and so on.
...
just may be needed to be accessed for other reasons by the attacker.
...
which include monitoring of activity such as communication across the network.
...
address provided and click connect.

![Alt Text](/assets/img/rdp.JPG)

Enter the credential provided where the username is `Analysis` and password is
`Tryhackme123!`
...
target machine’s windows desktop.

![Alt Text](/assets/img/rdp3.JPG)

I on the other hand used the Linux way because after attempting to follow the
process using the virtual machine provided, I got an error.

![Alt Text](/assets/img/rdp4.JPG)

I had to first connect the try hack me VPN using OpenVPN. For Linux I was
advised to use Remmina which I found through
**Applications>Internet>Remmina**. I typed in the IP address of the target
machine.
...
folder, there’s three applications (aws, NetLogo and vlc).
...
that I found for each application.
...
packed with Microsoft Visual C++ 6.0 DLL as shown below.
...
behaviors and ultimately with the aim of achieving infection.
...
with and found it to be FSG 1.0 -> dulek/xt.
...
is not able to automatically de-obfuscate them.
...
those that make use of the msi library, and they were found to be 9 in total.
